...
config = configparser.ConfigParser(); config.read('include/config.ini')
...
def main():
    for file in os.listdir(config.get("DEFAULT", "DataFolderName")):
        if (file.endswith(".zip")):
            zip_file_path = f"{config.get('DEFAULT', 'DataFolderName')}/{file}"; csv_file_path = os.path.splitext(zip_file_path)[0]
